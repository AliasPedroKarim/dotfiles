...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/bar/crypto.py) \","
  echo -n "\"color\":\"#000000\","
...
myip_public() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/bar/ip.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/bar/disk.py)%\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/bar/memory.py)%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/bar/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/bar/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
do
  ...
  if [[ $line == *"name"*"id_vpn"* ]]; then
    alacritty -e ~/.config/i3/bar/click_vpn.sh &

  # CHECK UPDATES
  elif [[ $line == *"name"*"id_systemupdate"* ]]; then
    alacritty -e ~/.config/i3/bar/click_checkupdates.sh &
  ...
  # TIME
    ...
    alacritty -e ~/.config/i3/bar/click_time.sh &
